...
--                                                                       --
---------------------------------------------------------------------------
-- Ethan Schoonover <es@ethanschoonover.com> @ethanschoonover            -- https://www.youtube.com/watch?v=70IxjLEmomg [YouTube-XMonad Demo]
-- https://github.com/altercation                                        -- .xmonad_ethan_schoonover
-- cp -fsr ~/.xmonad_ethan_schoonover/**/* ~/.monad/
---------------------------------------------------------------------------
-- current as of XMonad 0.12

------------------------------------------------------------------------}}}
-- ~/config_github/app/mv_cp_mkdir_rm_rmdir/readme.org
-- config_github sync
{-

(async-shell-command "mkdir ~/.xmonad/")
(async-shell-command "cp -fsr ~/.xmonad_ethan_schoonover/** ~/.xmonad/")

(async-shell-command "mkdir ~/config_github/app/xmonad/.xmonad_ethan_schoonover/")
(async-shell-command "cp -fl ~/.xmonad_ethan_schoonover/xmobar.conf ~/config_github/app/xmonad/.xmonad_ethan_schoonover/")

(async-shell-command "cp -fl ~/.xmonad_ethan_schoonover/xmonad.hs ~/config_github/app/xmonad/.xmonad_ethan_schoonover/")
...
cp ~/.xmonad_cp ~/.xmonad
cp ~/config_git/___.xmonad_cp/** ~/.xmonad/
...
wsGGC   = "GGC"
-}
ws_1_web       = "1:WEB"  -- main , text chat media syst
ws_2_emacs     = "2:TXT" -- text emacs
ws_3_scheduler = "3:TIME" -- scheduler --
ws_4_greek     = "4:Ελλάς"  -- Hellas, ădis, f., = Ἑλλάς.
-- Hellēnes, um, m. (Ἕλληνες), Hellènes, Grecs

ws_5_latin     = "5:SPQR" -- latin
ws_6_mpv       = "6:MED" -- Media mpv
ws_7_dolphin   = "7:FOLDER" -- dolphin
ws_8_TMP       = "8:TMP"
ws_9_dolphin   = "9:SYS" -- syst
wsTMP  = "TMP"

-- myWorkspaces = [wsGEN, wsWRK, wsWRK2, wsSYS, wsMON, wsFLOAT, wsRW, wsTMP]
myWorkspaces = [ws_1_web, ws_2_emacs, ws_3_scheduler, ws_4_greek, ws_5_latin, ws_6_mpv, ws_7_dolphin, ws_8_TMP]
projects :: [Project]
projects =

    [ Project   { projectName       = ws_1_web
                , projectDirectory  = "~/"
                , projectStartHook  = Nothing
                }

    , Project   { projectName       = ws_2_emacs
                , projectDirectory  = "~/"
                , projectStartHook  = Just $ do 
                                                spawnOn ws_2_emacs myTerminal
                }

    , Project   { projectName       = ws_3_scheduler
                , projectDirectory  = "~/"
                -- , projectStartHook  = Just $ do spawn "/usr/lib/xscreensaver/binaryring"
                , projectStartHook  = Just $ do spawnOn ws_3_scheduler "/usr/bin/scheduler"
...
-}
    ...
    , Project   { projectName       = ws_4_greek
                , projectDirectory  = "~/.xmonad"
                , projectStartHook  = Just $ do runInTerm "-name vix" "vim ~/.xmonad/xmonad.hs"
                                                spawnOn ws_4_greek myPDF1
                                                spawnOn ws_4_greek myTerminal
                }

    , Project   { projectName       = ws_5_latin
                , projectDirectory  = "~/"
                , projectStartHook  = Just $ do runInTerm "-name glances" "glances"
                                                spawnOn ws_5_latin "/usr/bin/goldendict"
                }

    , Project   { projectName       = ws_6_mpv
                , projectDirectory  = "~/Music/"
                , projectStartHook  = Just $ do spawnOn ws_6_mpv myTerminal
                                                spawnOn ws_6_mpv myBrowser
                }

    , Project   { projectName       = ws_7_dolphin
                , projectDirectory  = "~/"
                , projectStartHook  = Just $ do spawn myBrowser
                }

    , Project   { projectName       = ws_8_TMP
                , projectDirectory  = "~/"
...
projects =
    ...
    , Project   { projectName       = wsSYS
                , projectDirectory  = "~/"
                , projectStartHook  = Just $ do spawnOn wsSYS myPDF1
...
--LJP setting
myPDF2              = "evince /home/thinkpad/Desktop/great_books_09k_aristotle_2_politics.pdf"
myPDF1              = "evince ~/Desktop/an_etymology_of_latin_greek_2020_01_added.pdf -p 100"
{-
myPDF1              = "evince '~/Desktop/great_books_07k2_plato_소크라테스의 변명,크리톤,향연.pdf'"
...
-- xft:nanumgoth:size=11,monospace-11
--myFont      = "xft:nanumgoth:size=16,monospace-11"
--myBigFont   = "xft:nanumgoth:size=24,monospace-11"
--myWideFont  = "xft:nanumgoth:size=18,monospace-11"

--myFont      = "-*-nanumgoth-*-*-*-*-160-*-*-*-*-*-*"
--myBigFont   = "-*-nanumgoth-*-*-*-*-240-*-*-*-*-*-*"
--myWideFont  = "xft:nanumgoth:"
--            ++ "pixelsize=180:hinting=true"
...
myLayoutHook = showWorkspaceName
             $ onWorkspace ws_6_mpv floatWorkSpace
...
--     , ("M-x"                    , addName "Lock screen"                     $ spawn "xset s activate")
  ...
  --  , ("M1-S-r"                 , addName "Record screen - area select"     $ spawn "screencast area" )
    ...
    , ("M-c"                    , addName "NSP Chat"                        $ bindOn WS [(ws_2_emacs, namedScratchpadAction scratchpads "hangoutsWork"),
                                                                              ("", namedScratchpadAction scratchpads "hangoutsPersonal")])
    , ("M-t"                    , addName "NSP Tasks"                       $ bindOn WS [(ws_3_scheduler, namedScratchpadAction scratchpads "trelloWork"),
